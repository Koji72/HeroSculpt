    ...
    <div className="w-full bg-gradient-to-br from-blue-600 via-purple-600 to-indigo-700 p-4 overflow-auto">
      <div className="sheet max-w-xl w-full mx-auto bg-[#161c2b] border-4 border-blue-400 rounded-xl p-6 shadow-2xl overflow-y-auto" style={{ maxHeight: '70vh' }}>
        <div className="flex justify-between items-center mb-6">
          <h1 className="text-center text-3xl font-bold text-blue-400 drop-shadow-lg">Character Sheet</h1>
          <button
        ...
        <div className="group mb-4">
          <label htmlFor="nombre" className="block font-bold mb-1 text-blue-200">Character Name</label>
          <input type="text" id="nombre" name="nombre" value={modular.name} onChange={e => handleChange('name', e.target.value)} placeholder="e.g. Red Neon" disabled={!isEditing} className="w-full p-2 rounded bg-[#292f45] border-2 border-blue-400 text-white disabled:opacity-50 disabled:cursor-not-allowed" />
        </div>
        <div className="group mb-4">
          <label htmlFor="grado" className="block font-bold mb-1 text-blue-200">Power Level</label>
          <select id="grado" name="grado" value={modular.grado} onChange={e => handleChange('grado', e.target.value)} disabled={!isEditing} className="w-full p-2 rounded bg-[#292f45] border-2 border-blue-400 text-white disabled:opacity-50 disabled:cursor-not-allowed">
            <option value="G1">G1 - Civilian</option>
            <option value="G3">G3 - Vigilante</option>
            <option value="G5">G5 - Urban Hero</option>
            <option value="G7">G7 - National Hero</option>
            <option value="G10">G10 - Global Threat</option>
          </select>
        </div>
        <div className="group mb-4">
          <label className="block font-bold mb-1 text-blue-200">Attributes</label>
          <div className="grid grid-cols-3 gap-2">
            <div>
              <label htmlFor="str" className="text-blue-100 text-xs">Strength (STR)</label>
            ...
            <div>
              <label htmlFor="end" className="text-blue-100 text-xs">Endurance (END)</label>
            ...
            <div>
              <label htmlFor="agi" className="text-blue-100 text-xs">Agility (AGI)</label>
            ...
            <div>
              <label htmlFor="wil" className="text-blue-100 text-xs">Willpower (WIL)</label>
            ...
            <div>
              <label htmlFor="int" className="text-blue-100 text-xs">Intellect (INT)</label>
            ...
            <div>
              <label htmlFor="pre" className="text-blue-100 text-xs">Presence (PRE)</label>
        ...
        <div className="group mb-4">
          <label htmlFor="poderes" className="block font-bold mb-1 text-blue-200">Powers (comma-separated)</label>
          <input type="text" id="poderes" name="poderes" value={modular.poderes} onChange={e => handleChange('poderes', e.target.value)} placeholder="e.g. Flight, Super Strength" disabled={!isEditing} className="w-full p-2 rounded bg-[#292f45] border-2 border-blue-400 text-white disabled:opacity-50 disabled:cursor-not-allowed" />
        </div>
        <div className="group mb-4">
          <label htmlFor="ventajas" className="block font-bold mb-1 text-blue-200">Advantages (comma-separated)</label>
          <input type="text" id="ventajas" name="ventajas" value={modular.ventajas} onChange={e => handleChange('ventajas', e.target.value)} placeholder="e.g. Power Strike, Adaptability" disabled={!isEditing} className="w-full p-2 rounded bg-[#292f45] border-2 border-blue-400 text-white disabled:opacity-50 disabled:cursor-not-allowed" />
        </div>
        <div className="group mb-4">
          <label htmlFor="descripcion" className="block font-bold mb-1 text-blue-200">Description / Notes</label>
          <input type="text" id="descripcion" name="descripcion" value={modular.descripcion} onChange={e => handleChange('descripcion', e.target.value)} placeholder="Backstory, appearance, etc." disabled={!isEditing} className="w-full p-2 rounded bg-[#292f45] border-2 border-blue-400 text-white disabled:opacity-50 disabled:cursor-not-allowed" />
        </div>
        <div className="abilities-section bg-[#202a40] border-2 border-blue-400 rounded-lg mt-6 p-4">
          ...
          <div className="box-xo2 max-h-48 overflow-auto space-y-2">
            ...
            {isEditing && (
              <button type="button" className="mt-2 w-full bg-blue-700 hover:bg-blue-600 text-white font-bold py-2 rounded" onClick={handleAddAbility}>+ Add Ability</button>
            )}
          </div>
        </div>
      </div>
