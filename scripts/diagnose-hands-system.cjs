...
try {
  ...
  // ✅ CORREGIDO: Solo detectar eliminaciones por ID reales, no variables que contienen categorías
  // Buscar patrones como delete newParts[part.id] o delete newParts[currentHand.id]
  const idDeletes = appContent.match(/delete newParts\[[^P][^a][^r][^t][^C][^a][^t][^e][^g][^o][^r][^y][^\.][^\]]*\.id\]/g);
...
try {
  ...
  if (handsFunction) {
    ...
    // ✅ CORREGIDO: Buscar patrones específicos de eliminación y asignación
    ...
    // Verificar asignaciones usando categorías
    ...
    // Verificar que NO use asignaciones por ID
    const assignById = functionContent.includes('newParts[') && functionContent.includes('.id] =');
    if (assignById) {
      console.log(`   ❌ ADVERTENCIA: Se detectaron asignaciones por ID`);
    } else {
      console.log(`   ✅ No se detectaron asignaciones por ID`);
    }
